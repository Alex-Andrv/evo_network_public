...
import src.tss_sat as tss_sat
import src.tss_tdg as tss_tdg
...
class TSSProblem:
    ...
    def my_fit(self, vec):
        ...
        if n != self.nodes_count():
            raise ValueError(
                "vector length ({}) != agents number ({})".format(n, self.nodes_count())
            )
        ...
        while True:
            ...
            for i in cur_indices:
                ...
                for agent_to in self.dltm.graph[agent]:
                    ...
                    if (
                        not cur_vec[i_to]
                        and cur_infl[i_to] >= self.dltm.agents[agent_to]
                    ):
    ...
    def solve_using_1p1(
        self, stop_criteria, mutation=genetic.non_increasing_default_mutation, seed=None
    ):
        return self.solve_abstract(
            lambda: genetic.using_1p1(
                [1] * self.nodes_count(), self.fit, mutation, stop_criteria
            ),
            seed,
        )

    def solve_using_genetic_algorithm(
        self,
        stop_criteria,
        mutation=genetic.non_increasing_default_mutation,
        crossover=genetic.crossover,
        seed=None,
    ):
        return self.solve_abstract(
            lambda: genetic.genetics_algorithm(
    ...
    def solve_using_1cl(self, lmbd, stop_criteria, seed=None):
        return self.solve_abstract(
            lambda: genetic.using_1cl(
                [1] * self.nodes_count(),
                self.fit,
                genetic.default_mutation,
                lmbd,
                stop_criteria,
            ),
            seed,
    ...
    def solve_using_custom_ga(self, l, h, g, stop_criteria, seed=None):
        solution, solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, None, None)
        )
        solution, metadata = self.solve_abstract(
            lambda: genetic.using_custom_ga(
                agents_to_vec(self, solution),
                self.fit,
                genetic.non_increasing_default_mutation,
                genetic.two_point_crossover,
                l,
                h,
                g,
                stop_criteria,
    ...
    def solve_using_tdg_and_then_1p1(self, d1, d2, stop_criteria, seed=None):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
        )
        solution, metadata = self.solve_abstract(
            lambda: genetic.using_1p1(
                agents_to_vec(self, tdg_solution),
                self.fit,
                genetic.non_increasing_default_mutation,
                stop_criteria,
            ),
    ...
    def enhance_solution(
        self, solution, stop_criteria, iterations, t_size, solutions_cache, old_solution
    ):
        ...
        while (
            iterations < stop_criteria.get_iteration_count()
            and best_fit < self.threshold
        ):
            drop_active_element = {
                el for el in active_agent if random.random() < (1 / len(active_agent))
            }
            ...
            curr_fit, curr_vector, _ = self.my_fit(
                agents_to_vec(self, new_active_agent)
            )
            iterations += 1
        ...
        return (
            (active_agent, iterations)
            if best_fit >= self.threshold
            else (set(old_solution), iterations)
        )
    ...
    def iter_descent_v3(self, stop_criteria, init_solution, iter_epoch, t_size):
        ...
        while len(solution) > 0 and (iterations < stop_criteria.get_iteration_count()):
            ...
            solution, iterations = self.enhance_solution(
                new_solution,
                stop_criteria,
                iterations,
                t_size,
        ...
        all_solutions = [
            list(zip(keys, values)) for keys, values in solutions_cache.items()
        ]
        # import torch
    ...
    def iter_descent_v2(self, stop_criteria, init_solution, iter_epoch, t_size):
        ...
        for agent_id in self.dltm.agents.keys():
            ...
            for agent_id_to in self.dltm.graph[agent_id]:
                inf[agent_id] += (
                    self.dltm.infl[(agent_id, agent_id_to)]
                    / self.dltm.agents[agent_id_to]
                )
    ...
    def iter_descent(self, stop_criteria, init_solution, iter_epoch, t_size):
        ...
        inf = {}
        for agent_id in self.dltm.agents.keys():
            inf[agent_id] = tss_tdg.compute_influence(
                self.dltm, agent_id, self.dltm.agents.copy(), set(), 1000
            )
    ...
    def iter_descent_v4(
        self, stop_criteria, init_solution, iter_epoch, t_size, limit=50
    ):
        solution = set(init_solution)
        iterations = 0
        ...
        while len(solution) > 0 and (iterations < stop_criteria.get_iteration_count()):
            for _ in range(iter_epoch):
                ...
                while (idx < len(del_agent_id)) and (idx < limit):
                    ...
                    potential_new_solution = [
                        x for x in solution if x != potential_del_agent_id
                    ]
                    new_fit, _, _ = self.my_fit(
                        agents_to_vec(self, potential_new_solution)
                    )
                    if new_fit > best_fit:
            ...
            for _ in range(iter_epoch):
                ...
                for agent_id in solution:
                    ...
                    for agent_to in self.dltm.graph[agent_id]:
                        del_intf = (
                            infl[self.dltm.agent_to_ord[agent_to]]
                            - self.dltm.infl[(agent_id, agent_to)]
                        )
    ...
    def iter_descent_v6(self, stop_criteria, init_solution, iter_epoch, t_size):
        ...
        while not (
            (
                stop_criteria.is_iteration_count()
                and iterations >= stop_criteria.get_iteration_count()
            )
            or (
                stop_criteria.is_stagnation_count()
                and stagnations >= stop_criteria.get_stagnation_count()
            )
        ):
            ...
            if new_fit >= self.threshold:
                ...
                if (self.threshold - new_fit) * ema_avg_ascent < ema_avg_descent:
                    ...
                    solution, new_iterations = self.enhance_solution(
                        new_solution,
                        stop_criteria,
                        iterations,
                        t_size,
                        cache,
                        vec_to_agents(self, cur_vec),
                    )
                    # TODO нужно пересчитываь current fit
                    new_vec = agents_to_vec(self, solution)
                    cur_fit, _, _ = self.my_fit(new_vec)
                    ema_avg_ascent = (
                        0.2 * (new_iterations - iterations) + 0.8 * ema_avg_ascent
                    )
    ...
    def solve_using_tdg_and_then_iter_descent_IMP(
        self, d1, d2, stop_criteria, iter_epoch=1, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
        )
    ...
    def solve_using_tdg_and_then_iter_descent_v3(
        self, d1, d2, stop_criteria, iter_epoch=1, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
        ...
        random.seed(seed)
        t_size = []
        new_solution, time = exec_with_time(
            lambda: self.iter_descent_v3(
                stop_criteria, tdg_solution, iter_epoch, t_size
            )
        )
        metadata = {"time": tdg_solution_time + time, "t_size": t_size}
        return new_solution, metadata

    def solve_using_tdg_and_then_iter_descent_v4(
        self, d1, d2, stop_criteria, iter_epoch=1, limit=50, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
        )
        ...
        t_size = []
        new_solution, time = exec_with_time(
            lambda: self.iter_descent_v4(
                stop_criteria, tdg_solution, iter_epoch, t_size, limit
            )
        )
        metadata = {"time": tdg_solution_time + time, "t_size": t_size}
        return new_solution, metadata

    def solve_using_tdg_and_then_iter_descent_v5(
        self, d1, d2, stop_criteria, iter_epoch=1, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
    ...
    def solve_using_tdg_and_then_iter_descent_v6(
        self, d1, d2, stop_criteria, iter_epoch=1, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
    ...
    def solve_using_tdg_and_then_iter_descent_v2(
        self, d1, d2, stop_criteria, iter_epoch=1, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
    ...
    def solve_using_tdg_and_then_iter_descent(
        self, d1, d2, stop_criteria, iter_epoch=1, seed=None
    ):
        tdg_solution, tdg_solution_time = exec_with_time(
            lambda: tss_tdg.solve(self, d1, d2)
        ...
        t_size = []
        new_solution, time = exec_with_time(
            lambda: self.iter_descent(stop_criteria, tdg_solution, iter_epoch, t_size)
        )
        metadata = {"time": tdg_solution_time + time, "t_size": t_size}
        return new_solution, metadata

    def solve_using_tdg_and_then_doerr_1p1(
        self, d1, d2, beta, stop_criteria, seed=None
    ):
    ...
    def solve_using_sat(
        self,
        pb_encoding=EncType.seqcounter,
        ...
        def do_solve():
            ...
            return (
                right_solution
                if right_solution
                else tss_sat.solve_tss(self, right, pb_encoding, sat_solver)
            )

        solution, time = exec_with_time(lambda: do_solve())
        return solution, {"time": time}

    def solve_using_sat_with_enhance_solution(
        self,
        stop_criteria,
        seed=None,
        pb_encoding=EncType.seqcounter,
