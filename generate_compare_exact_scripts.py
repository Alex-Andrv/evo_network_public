...
# Determine the absolute working directory where the script runs
...
if __name__ == '__main__':
    tss_name, dltm = task_name, read_dltm(f'{working_dir}/generated_graph/{experiment_series}/{task_name}')
    ...
    with open(f'{working_dir}/{results_dir}/{solver_name}_{task_name}.csv', 'w') as f:
        ...
        for i in range(20):
            ...
            f.flush()
""".replace("{working_dir}", working_dir).replace("{results_dir}", results_dir)

if __name__ == '__main__':
    solvers = [
        ('exact_sat', "lambda problem, seed: problem.solve_using_sat()"),
        ('tdg', "lambda problem, seed: problem.solve_using_tdg(None, None)"),
        ('1+1[10000]', "lambda problem, seed: problem.solve_using_1p1(tss_stop_criteria.by_iteration_count(10000), seed=seed)"),
        ('tdg_1+1[10000]', "lambda problem, seed: problem.solve_using_tdg_and_then_1p1(None, None, tss_stop_criteria.by_iteration_count(10000), seed=seed)"),
    ]
    tasks = [
    ...
    for solver_name, run_scrip in solvers:
        for task in tasks:
            ...
            safe_full_name = full_name.replace("&", "_").replace("[", "_").replace("]", "_").replace(".", "_")
            ...
            with open(os.path.join(scripts_dir, py_filename), "w") as py_script:
                script_content = (
                    python_script_template_head.format(task, solver_name, run_scrip, experiment_series)
                    + python_script_template_body
                )
